...
- **2026-09-01** 📊 **Los resúmenes A-share y las revisiones QMT dejan de quemar cincuenta llamadas a herramientas**: peticiones como「analizar el mercado A de hoy」o「今日交易复盘」inyectan SOP dedicados (`a-share-daily-brief`, `broker-trade-review`) en lugar de repetir `search_symbol`, cambiar fuentes de `get_market_data` o recurrir a `web_search`. Con throttling de Eastmoney, el ranking sectorial y los screeners A-share usan **akshare** y el flujo northbound **tushare**; los índices canónicos prefieren **Tencent** (menos 404 de QMT). El bucle **cachea** ventanas exitosas de `get_market_data`, omite pasos duplicados del brief y **bloquea** herramientas opcionales de mercado/noticias en revisiones de bróker salvo petición explícita, exigiendo un pie **数据获取状态** si se intentaron.
...
- **2026-08-28** 📈 **Hub de mercados y activo bruto vs neto en cartera**: la nueva página `/markets` incluye lista local, velas, calor sectorial, pestañas **Narrativas** y **Anomalías** (insights por reglas OHLCV), noticias, pares, resultados y evaluaciones LLM bajo demanda (`POST /api/markets/{symbol}/assessment`, límite de cinco minutos por símbolo). La REST de mercados añade CRUD de lista, `GET /api/markets` resumen, `GET /api/markets/{symbol}` detalle con insights embebidos, y `…/news`, `…/earnings`, `…/peers` — A-shares vía QMT Bridge para ticks, nombres chinos en lote y barras históricas con descarga automática si el caché local está vacío. La cartera separa **activo total bruto** de **activo neto** tras deuda de financiación — crédito QMT (`m_dBalance` / `m_dAssureAsset`) y margen Futu (`total_assets` + `debt_cash`) — con **cambio diario bruto/neto** frente al último snapshot del día hábil anterior y tooltips en cada métrica; «incluir efectivo» solo afecta la cobertura de valoración. El conector QMT normaliza marcas de barras y expone `name` en posiciones; `sector_tool` resuelve pares del mismo sector en A-shares.
<details>
