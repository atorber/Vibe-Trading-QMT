...
- **2026-08-28** 📈 **마켓 허브와 포트폴리오 총자산/순자산 분리**: 새 `/markets` 페이지에 로컬 관심목록, 캔들차트, 섹터 열도, **내러티브**·**이상** 탭(OHLCV 규칙 인사이트), 뉴스, 동종업체, 실적, 온디맨드 LLM 평가(`POST /api/markets/{symbol}/assessment`, 종목당 5분 레이트 리밋)를 추가했습니다. 마켓 REST는 관심목록 CRUD, `GET /api/markets` 개요, `GET /api/markets/{symbol}` 상세(인사이트 포함), `…/news`·`…/earnings`·`…/peers`를 제공하며, A주는 QMT Bridge로 틱·중국어 이름 일괄 조회·과거 봉을 가져오고 로컬 캐시가 비면 자동 다운로드합니다. 포트폴리오는 **총자산**과 융자 부채 차감 후 **순자산**을 분리 표시합니다 — QMT 신용(`m_dBalance` / `m_dAssureAsset`)과 Futu 마진(`total_assets` + `debt_cash`) — 전 거래일 마지막 스냅샷 대비 **당일 총자산/순자산 변동**과 각 지표 툴팁이 있으며, 「현금 포함」은 평가 커버리지에만 영향을 줍니다. QMT 커넥터는 K선 타임스탬프 정규화와 포지션 `name`도 추가했고, `sector_tool`은 A주 동종업체를 해석합니다.
<details>
