...
- **2026-09-02** 📊 **CSI300 알파 벤치에 QMT Bridge / akshare 가격 폴백, 포트폴리오 자산 이력에 총자산·순자산 이중 곡선**: Tushare dj_factor 권한이 없거나 CSI300 패널이 너무 얇으면 벤치 로더가 **QMT Bridge 전복권** → **akshare** 순으로 폴백합니다. 5종 미만 패널·손상 캐시는 거부하고 스킵 사유를 UI에 표시합니다. /portfolio 자산 이력은 **총자산**과 **순자산**을 함께 그립니다. 백테스트 보고서 목록에서 긴 설명이 수익률 지표를 밀어내지 않습니다.

- **2026-09-01** 📊 **A주 브리프와 QMT 거래 리뷰가 50번의 도구 호출을 헛돌지 않습니다**: 「오늘 A주 분석」「오늘 거래 리뷰」류 프롬프트는 전용 SOP(`a-share-daily-brief`, `broker-trade-review`)를 자동 주입하고, `search_symbol` 루프·`get_market_data` 소스 교체·`web_search` 우회를 막습니다. 동방재부 스로틀 시 섹터 순위와 A주 스크리너는 **akshare**, 북향 자금은 **tushare**로 폴백합니다. 주요 지수 코드는 **Tencent**를 우선해 QMT 404를 줄입니다. 루프는 성공한 `get_market_data` 창을 **캐시**하고 브리프 단계 중복을 건너뜁니다. 브로커 리뷰 중에는 사용자가 요청하지 않으면 선택적 시장/뉴스 도구를 **차단**하며, 시도했다면 **数据获取状态** 표가 필요합니다.
