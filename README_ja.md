...
- **2026-09-01** 📊 **A株ブリーフと QMT 取引レビューが 50 回のツール呼び出しで空回りしなくなった**：「今日の A 株を分析」「今日の取引レビュー」系のプロンプトは、専用 SOP（`a-share-daily-brief`、`broker-trade-review`）を自動注入し、`search_symbol` のループ、`get_market_data` のソース切替、`web_search` への逃げを避けます。東財のスロットル時はセクター順位と A 株スクリーナーが **akshare**、北向資金が **tushare** にフォールバック。主要指数コードは **Tencent** を優先し QMT 404 を減らします。ループは成功した `get_market_data` ウィンドウを**キャッシュ**し、ブリーフ手順の重複をスキップ。ブローカーレビュー中はユーザーが求めない限りオプションの市場/ニュース系ツールを**ブロック**し、試行した場合は **数据获取状态** フッターを要求します。
...
- **2026-08-28** 📈 **マーケットハブとポートフォリオの総資産/純資産の分離**：新しい `/markets` ページにローカルウォッチリスト、ローソク足、セクター熱度、**ナラティブ**と**アノマリー**タブ（OHLCV ベースのルール洞察）、ニュース、ピア、決算、およびオンデマンドの LLM 評価（`POST /api/markets/{symbol}/assessment`、銘柄ごと 5 分レート制限）を追加。マーケット REST はウォッチリスト CRUD、`GET /api/markets` 概要、`GET /api/markets/{symbol}` 詳細（洞察埋め込み）、`…/news`・`…/earnings`・`…/peers` を提供——A 株は QMT Bridge 経由でティック、中国語名称一括取得、履歴バーを取得し、ローカルキャッシュが空なら自動ダウンロード。ポートフォリオは融資負債控除後の**純資産**とブローカー報告の**総資産**を分離表示——QMT 信用（`m_dBalance` / `m_dAssureAsset`）と Futu マージン（`total_assets` + `debt_cash`）——前営業日最終スナップショット対比の**当日総資産/純資産変動**と各指標ツールチップ付き。「現金を含める」は評価カバレッジのみに影響。QMT コネクタは K 線タイムスタンプ正規化とポジション `name` も追加；`sector_tool` は A 株の同業ピアを解決。
<details>
