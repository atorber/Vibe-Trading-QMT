...
- **2026-09-02** 📊 **CSI300 アルファベンチに QMT Bridge / akshare の価格フォールバック、ポートフォリオ資産履歴に総資産と純資産の二重線**：Tushare dj_factor が使えない、または CSI300 パネルが薄すぎる場合、ベンチローダーは **QMT Bridge 前復権** → **akshare** の順にフォールバック。5 銘柄未満のパネルや壊れたキャッシュは拒否し、スキップ理由を UI に表示。/portfolio の資産履歴は **総資産** と **純資産** を同時表示。バックテストレポート一覧で長い説明がリターン指標を押しつぶさなくなりました。
