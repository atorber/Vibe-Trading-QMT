...
- **2026-09-02** 📊 **CSI300 alpha bench gets QMT Bridge / akshare price fallbacks, and portfolio history plots gross and net assets**: when Tushare `adj_factor` is missing or the CSI300 panel is too thin, the bench loader falls back to **QMT Bridge front-adjusted bars**, then **akshare**; corrupt universe caches are rejected and skip reasons surface in the UI. `/portfolio` asset history now draws **total assets** and **net assets** together; the backtest report list no longer squeezes return metrics beside long descriptions.
