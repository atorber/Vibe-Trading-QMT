...
- **2026-09-01** 📊 **A-share briefs and QMT trade reviews stop burning fifty tool turns**: prompts like「analyze today's A-share market」or「今日交易复盘」now auto-inject dedicated SOPs (`a-share-daily-brief`, `broker-trade-review`) instead of looping on `search_symbol`, swapping `get_market_data` sources, or falling back to `web_search`. Eastmoney throttling gets **akshare/tushare fallbacks** for sector ranking, A-share screeners, and northbound flow; canonical index codes prefer **Tencent** first (fewer QMT Bridge 404s). The agent loop **caches** successful `get_market_data` windows, skips duplicate brief steps, blocks optional market/news tools during broker reviews unless the user asks, and the broker SOP requires a **数据获取状态** footer when optional context tools were tried.
...
- **2026-08-28** 📈 **Markets hub and portfolio gross vs net equity**: The new `/markets` page ships a local watchlist with candlesticks, sector heat, **Narratives** and **Anomalies** tabs (rule-based OHLCV insights), news, peers, earnings, and on-demand LLM assessments (`POST /api/markets/{symbol}/assessment`, five-minute per-symbol rate limit). Markets REST adds watchlist CRUD, `GET /api/markets` overview, `GET /api/markets/{symbol}` detail with embedded insights, plus `…/news`, `…/earnings`, and `…/peers` — QMT Bridge backs A-share ticks, batch Chinese names, and historical bars with auto-download when local cache is empty. Portfolio now reports **gross total assets** separately from **net assets** after financing debt — QMT credit (`m_dBalance` / `m_dAssureAsset`) and Futu margin (`total_assets` + `debt_cash`) — with **today's total and net asset change** vs the previous trading day's last snapshot and hover tips on each headline metric; the "include cash" toggle affects valuation coverage only. QMT connector also normalizes K-line timestamps and surfaces position `name` fields; `sector_tool` resolves same-industry A-share peers for the agent.
<details>
...
| **Browse a markets watchlist** | The `/markets` page: candlesticks, sector heat, narratives, anomalies, news, peers, earnings, and on-demand LLM assessments for symbols you track locally. |
| **Aggregate live holdings** | The `/portfolio` page: multi-broker snapshots with gross vs net equity, today's asset change, CSV export, and `portfolio_risk_xray` hooks. |
...
| **Gross vs net equity** | Headline **total assets** sum broker-reported gross balances; **net assets** subtract financing debt (margin loans). Account cards show gross with net as a subtitle when debt is nonzero. |
| **Today's change** | **Today's asset change** compares the current snapshot to yesterday's last complete refresh — total and net variants — covering market moves, trading, and cash flows. Shows — when no baseline exists. |
...
The Web UI adds a read-only **Markets** page for symbols you track in a local watchlist — separate from the multi-broker Portfolio view.

| Behavior | What you get |
|----------|--------------|
| **Local watchlist** | Add symbols from search; state persists in the browser and syncs through `GET`/`PUT`/`POST`/`DELETE /api/markets/watchlist`. |
| **Market context** | Candlesticks, sector heat, stock news, peer comparison, and earnings calendar tabs per selected symbol. |
| **Narratives & anomalies** | Rule-based session moves and volume/range spikes on dedicated tabs — same OHLCV + quote logic embedded in the detail API for assessments. |
| **On-demand assessment** | Click **Generate assessment** to call `POST /api/markets/{symbol}/assessment` — aggregates quotes, narrative, anomalies, news, peers, and earnings into one LLM summary (five-minute per-symbol rate limit, cached). |
| **REST surface** | `GET /api/markets` overview; `GET /api/markets/{symbol}` detail; `GET …/news`, `…/earnings`, `…/peers`; watchlist CRUD above. |
| **QMT Bridge quotes** | A-share symbols route through QMT Bridge for ticks, instrument detail, batch Chinese names, and historical bars — auto-downloading local cache when bars are empty. |
| **Agent parity** | Sector, news, and industry-peer tools used by the agent back the same endpoints the page calls. |
