...
- **2026-09-01** 📊 **ملخصات A-share ومراجعات QMT للتداول لم تعد تحرق خمسين استدعاء أداة بلا جدوى**: مطالبات مثل「تحليل سوق A اليوم」أو「今日交易复盘」تحقن الآن SOP مخصصة (`a-share-daily-brief`, `broker-trade-review`) بدل التكرار على `search_symbol` أو تبديل مصادر `get_market_data` أو اللجوء إلى `web_search`. عند خنق Eastmoney، يلجأ ترتيب القطاع وغربلة A-share إلى **akshare** وتدفق northbound إلى **tushare**؛ ورموز المؤشرات القياسية تفضّل **Tencent** أولًا (أقل 404 من QMT). الحلقة **تخزّن** نوافذ `get_market_data` الناجحة وتتخطى خطوات الملخص المكررة، وت**حجب** أدوات السوق/الأخبار الاختيارية أثناء مراجعة الوسيط ما لم يطلب المستخدم، مع اشتراط تذييل **数据获取状态** عند المحاولة.
...
- **2026-08-28** 📈 **مركز الأسواق وفصل إجمالي الأصول عن صافيها في المحفظة**: صفحة `/markets` الجديدة تضم قائمة مراقبة محلية، شموعاً، حرارة القطاعات، تبويبي **السرد** و**الشذوذ** (رؤى قواعد OHLCV)، أخباراً، أقراناً، أرباحاً، وتقييمات LLM عند الطلب (`POST /api/markets/{symbol}/assessment`، حد معدل خمس دقائق لكل رمز). REST للأسواق يضيف CRUD للقائمة، `GET /api/markets` نظرة عامة، `GET /api/markets/{symbol}` تفاصيل مع رؤى مدمجة، و`…/news` و`…/earnings` و`…/peers` — أسهم A عبر QMT Bridge للتيكات وأسماء صينية مجمعة وأشرطة تاريخية مع تنزيل تلقائي عند فراغ الذاكرة المحلية. تعرض المحفظة **إجمالي الأصول** منفصلاً عن **صافي الأصول** بعد ديون التمويل — ائتمان QMT (`m_dBalance` / `m_dAssureAsset`) وهامش Futu (`total_assets` + `debt_cash`) — مع **تغير إجمالي/صافي اليوم** مقابل آخر لقطة ليوم التداول السابق وتلميحات لكل مقياس؛ مفتاح «تضمين النقد» يؤثر على تغطية التقييم فقط. موصل QMT يطبّع طوابع K-line ويعرض حقل `name` للمراكز؛ `sector_tool` يحل أقران A-share في القطاع.
<details>
